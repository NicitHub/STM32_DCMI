...
uint8_t   jpeg_data_buf = 65535; //the jpeg buffer is set behind the LCD layer1(480*272,RGB565 mode) in the SDRAM
...
{ 
    ...
    OV5640_POWER_ON; // Power on -> Ruhezustand: 1, normaler Betrieb: 0

	HAL_Delay(30);         

	reg=OV5640_RD_Reg(OV5640_CHIPIDH); // default: 0x56
	reg<<=8;
	reg|=OV5640_RD_Reg(OV5640_CHIPIDL); // default: 0x40
	if(reg!=OV5640_ID)
	{
		printf("ID: %d \r\n",reg);
		return 1;
...
void jpeg_test(uint8_t jpg_size)
{
        ...
        */
        ...
        HAL_DCMI_Start_DMA(&hdcmi, DCMI_MODE_SNAPSHOT, jpeg_data_buf, jpeg_buf_size/4);
        //HAL_DCMI_Start_DMA(&hdcmi, DCMI_MODE_SNAPSHOT, (uint32_t)jpeg_data_buf, jpeg_buf_size/4 );
